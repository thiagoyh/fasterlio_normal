...
void LaserMapping::Run() {
    ...
    ObsModel(state_point_, measures_.lidar_end_time_);
    // update local map
...
void LaserMapping::ObsModel(state_ikfom &s, double timestamp) {
    ...
    Eigen::Quaternionf R_wl = (s.rot * s.offset_R_L_I).cast<float>();
    Eigen::Vector3f t_wl = (s.rot * s.offset_T_L_I + s.pos).cast<float>();
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
        ...
        if (point_selected_surf_[i]) {
            ...
            if (valid_corr) {
                ...
                point_world.normal_x = plane_coef_[i].x();
                point_world.normal_y = plane_coef_[i].y();
                point_world.normal_z = plane_coef_[i].z();
            }
        }
    });
    char buffer[50];
    std::sprintf(buffer, "%.2f", timestamp);
    std::string timestamp_string = buffer;
    std::string pose_path = "/home/lyh/ws/result/20241002/pose.txt";
    std::string pcd_path = "/home/lyh/ws/result/20241002/pcd";
    static std::ofstream pose_file(pose_path, std::ios::app);

    pose_file << timestamp_string << " " << t_wl.x() << " " << t_wl.y() << " " << t_wl.z() << " "
              << R_wl.x() << " " << R_wl.y() << " " << R_wl.z() << " " << R_wl.w() << std::endl;

    pcl::PointCloud<pcl::PointXYZINormal>::Ptr points_to_save(new(pcl::PointCloud<pcl::PointXYZINormal>));
    for (int k = 0; k < point_selected_surf_.size(); k++) {
        pcl::PointXYZINormal p = scan_down_world_->points[k];
        points_to_save->push_back(p);
    }
    std::string cur_pcd_path = pcd_path + "/" + timestamp_string + ".pcd";

    pcl::PCDWriter pcd_writer;
    pcd_writer.writeBinary(cur_pcd_path, *points_to_save);
}
